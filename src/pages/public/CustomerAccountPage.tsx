...
export function CustomerAccountPage() {
  ...
  // Estados derivados para el teléfono
  ...
  // Al cargar el teléfono guardado, separarlo en prefijo y número
  useEffect(() => {
  ...
  // Calcular completitud del perfil (simple, basado en campos llenos)
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(160deg,#fcf8f2_0%,#f5ede3_40%,#fcf8f2_100%)]">
      <div className="mx-auto max-w-7xl px-4 py-8 lg:px-8 lg:py-12">
        {/* Hero section mejorada con barra de progreso de perfil */}
        <section className="relative overflow-hidden rounded-3xl border border-atarah-gold-300/60 bg-white shadow-xl shadow-atarah-gold-200/20">
          ...
          <div className="relative grid gap-0 lg:grid-cols-[1.2fr_0.8fr]">
            ...
            <div className="px-6 py-10 sm:px-10 lg:px-12 lg:py-12">
              <div className="inline-flex items-center gap-2 rounded-full bg-atarah-cream-100 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.2em] text-atarah-wine-900 border border-atarah-gold-300/50">
                <Sparkles className="size-4 shrink-0 self-center" />
                Espacio personal
              </div>
              <h1 className="mt-6 font-display text-4xl font-bold leading-tight text-atarah-wine-900 sm:text-5xl">
                {getGreeting()}, <span className="text-atarah-gold-700">{displayName.split(' ')[0]}</span>
              </h1>
              <p className="mt-4 max-w-xl text-base leading-relaxed text-atarah-charcoal-600">
                Gestiona tu perfil, revisa tus pedidos y continúa descubriendo nuestra colección de uniformes.
              </p>
              <div className="mt-8 flex flex-wrap gap-4">
                <Link to="/catalogo">
                  <Button size="lg" className="shadow-lg shadow-atarah-wine-900/20" leftIcon={<ShoppingBag className="size-4" />}>
                    Seguir comprando
                  </Button>
                </Link>
              </div>
            ...
            <div className="flex flex-col justify-between border-t border-atarah-gold-300/60 bg-gradient-to-br from-atarah-wine-950 to-atarah-wine-900 px-6 py-8 text-white lg:border-l lg:border-t-0 sm:px-10 lg:px-8">
              <div>
                <div className="flex items-center gap-5">
                  <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-full border-2 border-atarah-gold-300/60 bg-white/10 text-2xl font-bold text-atarah-gold-300 shadow-lg shadow-black/10">
                    {initials}
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-[0.2em] text-atarah-gold-300/80">Bienvenido</p>
        ...
        <section className="mt-8 grid grid-cols-2 gap-4 md:grid-cols-4">
          {[
            ...
            <Card
              key={stat.label}
              className="relative overflow-hidden border-0 bg-white p-5 shadow-md shadow-atarah-gold-200/20 transition hover:-translate-y-1 hover:shadow-lg"
            >
              <div className="absolute -right-4 -top-4 h-20 w-20 rounded-full opacity-20" style={{ backgroundColor: 'currentcolor' }} />
              <div className="relative flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-atarah-charcoal-500">{stat.label}</p>
                  <p className="mt-1 text-3xl font-bold text-atarah-wine-900">{stat.value}</p>
                </div>
                <div className={`rounded-2xl p-3 ${stat.color}`}>
                  <stat.icon className="size-5 shrink-0 self-center" />
                </div>
              </div>
            </Card>
          ))}
        ...
        <section className="mt-8 grid gap-6 lg:grid-cols-[1fr_1.2fr]">
          {/* Bloque de perfil editable con toggle de edición */}
          <div className="space-y-6">
            <Card className="border-0 bg-white p-6 shadow-md shadow-atarah-gold-200/20">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h2 className="font-display text-2xl font-bold text-atarah-wine-900">Datos de envío</h2>
                ...
                <div className="flex gap-2">
                  {!isEditingProfile ? (
                    <Button
                      ...
                      className="border-atarah-gold-400/60"
                    >
                      Editar
                    </Button>
                  ) : (
                ...
                <div className="sm:col-span-2">
                  ...
                  <div className="grid grid-cols-[88px_minmax(0,1fr)] gap-2">
                    <Select
                      ...
                      className="w-full"
                      disabled={!isEditingProfile}
                      aria-label="C?digo de operadora"
                    >
            ...
            {/* Accesos rápidos */}
            <Card className="border-0 bg-white p-6 shadow-md shadow-atarah-gold-200/20">
              <h2 className="font-display text-xl font-bold text-atarah-wine-900">Accesos rápidos</h2>
              <div className="mt-4 grid gap-2">
                <Link
                  to="/catalogo"
                  className="flex items-center gap-3 rounded-2xl border border-atarah-gold-300/40 px-5 py-3 transition hover:bg-atarah-cream-100 hover:border-atarah-gold-400"
                >
                  <ShoppingBag className="size-5 text-atarah-wine-900" />
                  <span className="font-medium text-atarah-charcoal-900">Explorar catálogo</span>
                  <ArrowRight className="ml-auto size-4 text-atarah-gold-500" />
                </Link>
                <Link
                  to="/carrito"
                  className="flex items-center gap-3 rounded-2xl border border-atarah-gold-300/40 px-5 py-3 transition hover:bg-atarah-cream-100 hover:border-atarah-gold-400"
          ...
          {/* Historial de pedidos con filtro y diseño mejorado */}
          <Card className="flex flex-col border-0 bg-white p-6 shadow-md shadow-atarah-gold-200/20 lg:p-8">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.15em] text-atarah-gold-700">Historial</p>
                <h2 className="font-display text-3xl font-bold text-atarah-wine-900">Tus pedidos</h2>
              </div>
              <Link to="/catalogo">
                <Button size="sm" variant="outline" className="border-atarah-gold-400/60" leftIcon={<ShoppingBag className="size-4" />}>
                  Nuevo pedido
                </Button>
              </Link>
            </div>
            ...
            <div className="mt-4 flex-1">
              {accountQuery.isLoading ? (
                ...
                <div className="max-h-[580px] space-y-4 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-atarah-gold-200">
                  {filteredOrders.map((order) => (
                    <article
                      key={order.id}
                      className="group rounded-2xl border border-atarah-gold-300/40 bg-white p-5 shadow-sm transition hover:border-atarah-gold-400 hover:shadow-md"
                    >
                      ...
                      <div className="mt-4 grid grid-cols-3 gap-3">
                        <div className="rounded-xl bg-atarah-cream-50 px-3 py-2 text-center">
                          <p className="text-xs uppercase text-atarah-charcoal-500">Total</p>
                          <p className="font-semibold text-atarah-charcoal-900">{formatCurrency(order.total)}</p>
                        </div>
                        <div className="rounded-xl bg-atarah-cream-50 px-3 py-2 text-center">
                          <p className="text-xs uppercase text-atarah-charcoal-500">Pagado</p>
                          <p className="font-semibold text-atarah-charcoal-900">{formatCurrency(order.deposit)}</p>
                        </div>
                        <div className="rounded-xl bg-atarah-cream-50 px-3 py-2 text-center">
                          <p className="text-xs uppercase text-atarah-charcoal-500">Saldo</p>
                          <p className="font-semibold text-atarah-charcoal-900">{formatCurrency(order.balance)}</p>
                        </div>
                      </div>
                      {order.items && order.items.length > 0 && (
                        <div className="mt-4 space-y-3">
                          <h3 className="text-sm font-semibold uppercase tracking-[0.14em] text-atarah-charcoal-600">
                            Productos comprados
                          </h3>
