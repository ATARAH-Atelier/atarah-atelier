...
export const checkoutSchema = z.object({
  address: z.string().trim().min(6, 'Ingresa una direcci?n v?lida.').max(250, 'La direcci?n es demasiado larga.'),
  ...
  discount_code: z.string().trim().max(60, 'El c?digo es demasiado largo.'),
  email: z.union([z.literal(''), z.string().email('Ingresa un correo v?lido.')]),
  ...
  phone: z.string().trim().min(1, 'Ingresa un tel?fono.'),
  ...
  if (normalizedPhone.length !== 11 || !phonePrefixes.some((prefix) => normalizedPhone.startsWith(prefix))) {
    context.addIssue({
      ...
      message: 'Ingresa un tel?fono venezolano v?lido.',
      path: ['phone'],
    })
  }
